...
module.exports = () => Promise.race([new Promise((resolve, reject) => {
  ...
  psProc.on('data', (data) => {
    ...
    if (polled) {
      logger.debug(`Found current focus: ${polled.exeName}`)
      resolve(polled)
      psProc.end().then(() => logger.debug('Powershell process exited'))
    }
  })
